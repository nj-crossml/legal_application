...
def llm_compare(prompt_data) -> str:
    ...
    """
    # setting the key parameters to invoke OpenAI
    response = client.chat.completions.create(  # Use OpenAI's ChatCompletion
    ...
    # getting the response from OpenAI and parsing it to return to the end user
    answer = response['choices'][0]['message']['content']  # Adjusted for OpenAI response format
